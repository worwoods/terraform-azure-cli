...
ARG TERRAFORM_VERSION
RUN apt-get update && \
  apt-get install --no-install-recommends -y CURL="${CURL}" && \
  apt-get install --no-install-recommends -y ca-certificates="${CA-CERTIFICATES}" && \
  apt-get install --no-install-recommends -y unzip="${UNZIP}" && \
  apt-get install --no-install-recommends -y gnupg="${GNUPG}"
WORKDIR /workspace
...
RUN apt-get update && \
    ...
    apt-get install -y --no-install-recommends python3-pip="${PYTHON3-PIP}"
RUN python -m pip install --no-cache-dir --upgrade pip=="${PYPI_PIP_VERSION}" && \
    pip3 install --no-cache-dir setuptools=="${SETUPTOOLS}" && \
